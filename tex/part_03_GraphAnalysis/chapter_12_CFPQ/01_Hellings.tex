\section{Алгоритм Хеллингса}
	\tikzsetfigurename{CFPQ_Hellings_}
\label{sec:CFPQ_Hellings}
...
В результате слово можно представить в виде линейного графа следующим образом (в качестве примера рассмотрим слово $a a b b a b$ из предыдущей главы~\ref{sec:lin_CYK}):
\begin{center}
    \input{part_03_GraphAnalysis/chapter_12_CFPQ/figures/01_Hellings/01_Hellings_fig_01.tex}
\end{center}
...
Для примера~\ref{exmpl:CYK} на шаге инициализации матрица выглядит следующим образом:
\[
    \begin{pmatrix}
...
При этом можно ослабить ограничение на форму входной грамматики: она должна находиться в ослабленной Нормальной Форме Хомского~(\ref{def:wCNF}).
...
\begin{example}
    ...
    \mytodo{Явно уже использовался. Здесь тоже наверное на подразделы лучше}
    ...
    \begin{center}
        \input{part_03_GraphAnalysis/chapter_12_CFPQ/figures/01_Hellings/graph1.tex}
    \end{center}
    ...
    \mytodo{Жирный и серый сливаются}
    \[
        \begin{pmatrix}
            \varnothing & \{A\}       & \{S\}                        & \{S_1\}                               \\
            \varnothing & \varnothing & \{A, S_1\}                   & \{S\}                                 \\
...
\begin{example}
    ...
    \mytodo{pick по сути не используется}
