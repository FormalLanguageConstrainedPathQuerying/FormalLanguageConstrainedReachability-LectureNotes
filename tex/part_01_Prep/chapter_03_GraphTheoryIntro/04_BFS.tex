...
\begin{algorithm}
    ...
    \While{$\emph{current\_front} \neq \ \overline{0}$}{
        $\emph{visited} \leftarrow \emph{visited} \opAddFrom{\BbbB} \emph{current\_front}$ \tcp*{Записываем вершины из фронта в посещённые}
        $\emph{new\_front} \leftarrow \emph{current\_front} \mmultFrom{\BbbB} M$ \tcp*{Вычисляем вершины, достижимые за один шаг из текущего фронта}
        $\emph{current\_front} \leftarrow \emph{new\_front} \opAddFrom{\BbbM} \emph{visited}$ \tcp*{Формируем новый фронт с учётом уже посещённых вершин}
    }
    \KwRet{\emph{visited}}\;
...
\begin{example}
    ...
    \begin{align*}
        \emph{visited} & = \emph{visited} \opAddFrom{\BbbB} \emph{current\_front} \\
                       & =
        \begin{pmatrix}
            0 & 0 & 0 & 0
    ...
    \begin{align*}
        \emph{new\_front} & = \emph{current\_front} \mmultFrom{\BbbB} M \\
                          & =
        \begin{pmatrix}
            0 & 0 & 1 & 0
    ...
    \begin{align*}
        \emph{current\_front} & = \emph{new\_front} \opAddFrom{\BbbM} \emph{visited}
        \\
                              & =
        \begin{pmatrix}
            1 & 0 & 0 & 1
