...
\mycomment{Длинное определение: вводная фраза отделена от условий значительным объёмом вложенных itemize. Желательно разбить или переструктурировать.}
\begin{definition}[Полукольцо]
    Непустое множество $R$ с двумя бинарными операциями $\opAdd: R \times R \to R$ (обычно называют сложением) и $\opMult: R \times R \to R$ (принято называть умножением) называется \emph{полукольцом}, если выполнены следующие условия.
    \begin{enumerate}
        \item $(R, \oplus)$~--- это коммутативный моноид, нейтральный элемент которого~--- $\Bbbzero$. Для любых $a, b, c \in R$:
              ...
              \end{itemize}
        \item $(R, \opMult)$~--- это моноид, нейтральный элемент которого~--- $\Bbbone$. Для любых $a, b, c \in R$:
              \begin{itemize}
                  \item $(a \opMult b) \opMult c = a \opMult (b \opMult c)$
                  \item $\Bbbone \opMult a = a \opMult \Bbbone = a$
              \end{itemize}
        \item $\opMult$ дистрибутивно слева и справа относительно $\oplus$:
              \begin{itemize}
                  \item $a \opMult (b \oplus c) = (a \opMult b) \oplus (a \opMult c)$
                  \item $(a \oplus b) \opMult c = (a \opMult c) \oplus (b \opMult c)$
              \end{itemize}
        \item $\Bbbzero$ является \emph{аннигилятором} по умножению:
              \begin{itemize}
                  \item для любых $a \in R$ выполнено $\Bbbzero \opMult a = a \opMult \Bbbzero = \Bbbzero$
              \end{itemize}
    \end{enumerate}
\end{definition}

Если операция $\opMult$ коммутативна, то говорят о \emph{коммутативном полукольце}.
...
\begin{example}
    ...
    В качестве умножения возьмём конкатенацию множеств ($\opMult \equiv \odot$) и определим её следующим образом:
    ...
    Проверим, что $\algebraic{S}_{\mathit{str}} = (R, \cup, \odot)$ действительно полукольцо по нашему определению.
    \begin{enumerate}
