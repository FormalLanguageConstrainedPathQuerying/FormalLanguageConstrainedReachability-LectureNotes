...
Для $m = 1$ эта нормальная форма соответствует ослабленной нормальной форме Хомского для КС-грамматик (определение~\ref{def:wCNF}).

\mytodo{Описать общий алгоритм приведения MCFG к нормальной форме, а не только результат для конкретной грамматики.}
\begin{example}
