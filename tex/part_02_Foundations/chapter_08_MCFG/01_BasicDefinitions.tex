...
\label{sec:MCFGBasicDefinitions}
...
В зависимости от ограничений на использование переменных в правилах, выделяют следующие подклассы MCFG~\sidecite{SEKI1991191}.

\mytodo{Определение well-nested: соответствует ли формальное условие (с регулярным выражением) стандартному определению из Seki1991 / Kanazawa2009.}
...
\begin{example}
    ...
    \]
\end{example}
\mytodo{Добавить визуализацию (TikZ-схему) перекрещивания переменных для well-nested и не-well-nested правил.}
...
\]
\mytodo{Добавить ссылки на литературу для включений MCFL $\subsetneq$ PMCFL $\subsetneq$ simpleLMG = P.}
...
\mytodo{Привести MCFG для MIX (конструкция Salvati 2-MCFL), а не только ссылку.}
...
\mytodo{Дать формальное определение interleaved Dyck language и пояснить, почему он не является MCFL.}
...
\mytodo{Дать формальное определение трёхмерного языка Дика D³.}
\mytodo{Упомянуть weak equivalence MCFG и TAG (Tree Adjoining Grammars).}
