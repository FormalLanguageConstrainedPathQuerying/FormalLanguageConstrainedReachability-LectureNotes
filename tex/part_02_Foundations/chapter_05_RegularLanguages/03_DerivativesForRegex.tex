...
\label{sec:DerivativesForRegex}
\tikzsetfigurename{RegLang_Deriv_}

Производные для регулярных языков предложены Янушем Бжозовским в работе <<Derivatives of Regular Expressions>>~\sidecite{10.1145/321239.321249}.
Будучи достаточно удобным механизмом для решения ряда (в том числе прикладных) задач, производные активно исследовались и мы будем использовать работу~\sidecite{10.1017/S0956796808007090} как основу для нашего изложения.
...
\begin{itemize}
    \item $\mathit{IsNull}(\varepsilon) = \emph{true}$
    \item $\mathit{IsNull}(\varnothing) = \emph{false}$
    \item $\mathit{IsNull}(x) = \emph{false}$
    \item $\mathit{IsNull}(R_1 \cdot R_2) = \mathit{IsNull}(R_1) \ \& \ \mathit{IsNull}(R_2) $
    \item $\mathit{IsNull}(R_1 \mid R_2) = \mathit{IsNull}(R_1) \mid \mathit{IsNull}(R_2) $
    \item $\mathit{IsNull}(R^*) = \emph{true}$
\end{itemize}
...
\[
\mathit{IsNull}(\partial_{t_{n-1}}(\ldots(\partial_{t_0}(R))\ldots)).
\]
...
\begin{example}
    ...
    \[
        \mathit{IsNull}(\partial_b(\partial_b(\partial_a(a^*(a \mid (b \ b))^*)))).
    \]
    ...
    \end{align*}
    $\mathit{IsNull}((a \mid (b \ b))^*) = \emph{true}$ по определению.
