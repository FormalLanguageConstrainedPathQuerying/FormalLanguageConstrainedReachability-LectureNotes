...
\mytodo{Убрать дублирование определений: общее для k и частное для $k=1$. Дать сразу частное с замечанием об обобщении, либо построить от общего к частному без повторов.}
\begin{definition}
    \label{def:firstk}
...
\mytodo{Добавить алгоритм построения $\first[k]$ и $\follow[k]$ для произвольного $k$ в виде псевдокода (в окружении algorithm).}
...
\end{example}
\mytodo{При унификации: заменить грамматику примера на сквозную.}
